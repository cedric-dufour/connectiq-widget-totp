...
class PickerAccountsDelete extends Ui.Picker {
  ...
  function initialize() {
    ...
    Picker.initialize({
      :title => new Ui.Text({ :text => Ui.loadResource(Rez.Strings.titleAccountsDelete), :font => Gfx.FONT_TINY, :locX=>Ui.LAYOUT_HALIGN_CENTER, :locY=>Ui.LAYOUT_VALIGN_BOTTOM, :color => Gfx.COLOR_BLUE }),
      :pattern => [ oPattern ]
    });
  }
...
class PickerAccountsDeleteDelegate extends Ui.PickerDelegate {
  ...
  function onAccept(_amValues) {
    ...
    if(_amValues[0] != null) {
      ...
      App.Storage.deleteValue(Lang.format("ACT$1$", [s]));
    }
