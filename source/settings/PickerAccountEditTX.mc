...
class PickerAccountEditTX extends Ui.Picker {
  ...
  function initialize() {
    ...
    var iValue =
      ...
      ? ($.dictMyCurrentTotpAccount as Dictionary<String>)["TX"] as Number
      : 30;
    if(iValue < 10) {
      iValue = 10;
    }
    ...
    Picker.initialize({
        :title => new Ui.Text({
            :text => Ui.loadResource(Rez.Strings.titleAccountTX) as String,
            :font => Gfx.FONT_TINY,
...
class PickerAccountEditTXDelegate extends Ui.PickerDelegate {
  ...
  function onAccept(_amValues) {
    ...
    if(dictAccount != null) {
      dictAccount["TX"] = _amValues[0] as Number;
    }
    else {
      dictAccount = {
